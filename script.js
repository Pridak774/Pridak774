document.addEventListener("DOMContentLoaded", function () {
  ...
  /* ===== Typewriter Effect for Glitch Text ===== */
  ...
  /* ===== Remove Glitch Overlay after 4 Seconds (faster for mobile) ===== */
  setTimeout(function () {
  ...
  if (bgCanvas) {
    ...
    function connectParticles() {
      ...
      for (let i = 0; i < particles.length; i += skipFactor) {
        let connections = 0;
        for (let j = i + 1; j < particles.length && connections < connectionLimit; j += skipFactor) {
    ...
    function addGlitchEffect() {
      ...
      if (Math.random() < (isMobile ? 0.03 : 0.08)) {
        try {
          ...
          const width = isMobile ? Math.min(bgCanvas.width, 300) : bgCanvas.width;
          const x = isMobile ? Math.floor(Math.random() * (bgCanvas.width - width)) : 0;
  ...
  // Use your API key here. Replace the empty string with your own key from Hugging Face.
  const API_URL = "https://api-inference.huggingface.co/models/facebook/blenderbot-400M-distill";
  const API_TOKEN = ""; // <-- Insert your API token here
  ...
  async function queryAI(inputText) {
    ...
    try {
      ...
      assistantResponse.classList.remove("loading");
      return { error: true, message: "// Error: Connection failed to AI service." };
    }
  }
  ...
  if (assistantButton && assistantInput) {
    ...
    assistantButton.addEventListener("click", async () => {
      ...
      if (result.error) {
        assistantResponse.textContent = result.message || "// Error: Could not process request.";
      } else {
